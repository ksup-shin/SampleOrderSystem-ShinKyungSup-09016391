...
- [ ] Phase 3에서 `PRODUCING`이 된 주문이 `생산 라인 조회`의 "현재 처리 중"에 나오는지.
- [ ] 부족분/실생산량 계산이 `ceil(부족분/수율)` 공식과 일치하는지 손계산으로 검증.
- [ ] 여러 건 승인 후 대기 목록이 승인한 순서(FIFO)대로 나오는지.
- [ ] "진행" 명령(또는 tick 트리거)을 반복 입력해 진행율/남은 시간이 줄어들다가, 완료 시 주문 상태가 `CONFIRMED`로 바뀌고 재고가 늘어나는지, 다음 대기 항목이 처리 중으로 넘어가는지 확인.
