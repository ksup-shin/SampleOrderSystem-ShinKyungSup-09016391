...
- [x] 콘솔 화면에 상태별 건수/재고 현황이 실제로 출력되는지 Phase 7 콘솔 앱 구동으로 확인(모니터링 화면의 `[1] 주문량 확인`/`[2] 재고량 확인` 번호 선택). 다만 PRD 예시의 표/진행바(잔여율 바 등) 같은 시각적 포맷팅은 현재 딕셔너리 텍스트 수준이며, 미감 개선은 후속 과제로 남김.
