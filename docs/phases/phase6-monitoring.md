...
- [x] 콘솔 화면에 상태별 건수/재고 현황이 실제로 출력되는지 Phase 7 콘솔 앱 구동으로 확인(모니터링 화면의 `[1] 주문량 확인`/`[2] 재고량 확인` 번호 선택).
- [x] (UX 2차 검토 반영) 주문량 확인/재고량 확인 모두 `dict` 텍스트가 아니라 컬럼 제목이 있는 표로 나오는지 — `format_order_counts_table`/`format_stock_levels_table`로 구현, 단위 테스트 + 실제 콘솔 구동(`상태 | 건수`, `시료 ID | 이름 | 재고 | 수요 | 상태` 헤더 확인)으로 검증 완료 (잔여율 바 같은 그래픽 요소는 여전히 범위 밖).
