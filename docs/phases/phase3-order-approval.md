...
- [ ] **재고 충분**: 재고를 주문 수량 이상으로 맞춘 뒤 승인 → `CONFIRMED`로 즉시 전환, 재고는 그대로.
- [ ] **재고 부족**: 재고보다 큰 수량 주문 → 승인 화면에서 부족분/예상 소요시간 확인 → 승인 후 `PRODUCING`, 생산 큐에 등록됨(Phase 4 화면에서 확인).
- [ ] **거절**: `RESERVED` 주문 거절 → `REJECTED`로 전환, 이후 승인/거절 목록에 다시 나타나지 않음.
- [ ] 이미 처리된 주문을 다시 선택하면 목록에 없거나 에러로 막히는지 확인.
