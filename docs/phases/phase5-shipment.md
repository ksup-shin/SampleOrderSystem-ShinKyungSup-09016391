...
- [x] 콘솔 화면에서 번호/주문번호/고객/시료이름/수량 형식으로 표시되는지 — Phase 7 재작업(번호 선택 UX) 이후 `[1] ORD-000001 | S-001 | CustA | 200 | CONFIRMED` 형태로 인덱스 번호까지 함께 표시되고, `번호:` 입력으로 선택해 출고 처리됨을 확인. PRD 예시의 "번호(1,2,3) 선택" 요구사항을 그대로 충족.
- [x] (UX 2차 검토 반영) 출고 처리 후 해당 주문 정보가 표로 한 번 더 나오고 `상태 변경: CONFIRMED → RELEASE`가 명시적으로 보이는지, 목록/현황이 헤더 있는 표로 나오는지 — Phase 7 재작업에서 구현, 단위 테스트 + 실제 콘솔 구동으로 검증 완료.
