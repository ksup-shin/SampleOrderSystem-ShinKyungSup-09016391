...
- [x] 콘솔 화면에서 주문번호/고객/시료이름/수량 형식으로 표시되는지 — Phase 7 콘솔 앱 구동으로 확인(`list`/`ship` 명령 출력에 해당 필드가 모두 나타남). 다만 PRD 예시처럼 "번호(1,2,3 선택용 인덱스)"는 아직 없고 주문번호를 직접 입력받는 방식으로 단순화되어 있음 — 필요 시 후속 개선.
