...
- [x] `시료 관리 → 시료 등록`에서 `S-001`/"실리콘 웨이퍼-8인치"/0.5/0.92 등록 → 성공 메시지 확인.
- [x] 동일 ID 재등록 시도 → 에러로 거부.
- [x] `시료 조회` → ID/이름/평균 생산시간/수율/재고 5개 항목이 모두 표시되는지 확인.
- [x] `시료 검색`에서 "실리콘" 검색 → 매치, "없는이름" 검색 → 빈 결과.
- [x] `data/samples.json` 또는 `poc_json.admin_tool data/samples.json`의 `list`로 실제 저장 내용 확인.
